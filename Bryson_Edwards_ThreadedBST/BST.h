...
public:
  ...
  void printInOrder() {
	...
	BSTNode<Key, E> *current;
	  ...
	  current = root;
	  while(current->left() != NULL) // keep looping left
		current = current->left();
		  
	  while (current->right() != NULL) // loop through right
	  {
		  visit(current); // print
		  ...
		  {
			  current = current->right();
			  while (!current->getLeftIsThread()) // loop through the left thread again 
			  {
				  current = current->left();
			  }
		  }
	  }
	  visit(current); // make sure when print even the last one to the right
  }
  void printReverse() 
  {
	  // printInReverse is the same function as printInOrder except all the rights are changed to left and vice versa
	  BSTNode<Key, E> *current;

	  current = root;
	  while (current->right() != NULL) // loop through right first
		  current = current->right();

	  while (current->left() != NULL) // go through the lefts
	  {
		  visit(current); // print
		  if (current->getLeftIsThread())
			  current = current->left();
		  else
		  {
			  current = current->left(); // loop through lefts
			  while (!current->getRightIsThread())
			  {
				  current = current->right();
			  }
		  }
	  }
	  visit(current); // print even the last one 
  }
...
BSTNode<Key, E>* BST<Key, E>::inserthelp( BSTNode<Key, E>* root, const Key& k, const E& it) {
	...
	if (k < root->key())
	{
		...
		if (root->getLeftIsThread())
		{
			// Create Node
			BSTNode<Key, E>* newNode;
			newNode = new BSTNode<Key, E>(k, it, NULL, NULL);
			newNode->setRight(root, "thread"); // set the right node to the root
			newNode->setLeft(root->left(), "thread"); // set the left node to the left node of the root BEFORE we change root's left to newNode
			root->setLeft(newNode, "regular"); //New Node 
		}
		// if left is a branch, then go one more down the branch
		else 
		{
			root->setLeft(inserthelp(root->left(), k, it), "regular");
		}
	}
	else
	{
		// if right is thread, reassign it to a new node 
		if (root->getRightIsThread())
		{
			BSTNode<Key, E>* newNode;
			newNode = new BSTNode<Key, E>(k, it, NULL, NULL);
			newNode->setLeft(root, "thread");
			newNode->setRight(root->right(), "thread");
			root->setRight(newNode, "regular"); //New Node
		}
		// if right is not a thread and is a branch, then go one more down the branch
		else
		{
			root->setRight(inserthelp(root->right(), k, it), "regular");
		}
	}
...
printhelp(BSTNode<Key, E>* root, int level) const {	
	...
	if(!root->getLeftIsThread()) // Make sure left is branch
	...
	for (int i=0; i<level; i++)         // Indent to level
		cout << "  ";
	cout << root->key() << "\n";        // Print node value
	if(!root->getRightIsThread()) // Make sure right is branch
